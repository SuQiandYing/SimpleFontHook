...
- 为 Unity Mono/IL2CPP、BGI、Artemis 与 Artemis Legacy、KiriKiri、Entis、Softpal、Escude、
  Mirai、Majiro、DxLib、TinkerBell、TyranoScript 和 Ren'Py 提供兼容路径。
...
4. 启动程序，在字体选择器中选择字体并启用需要的功能。

字体选择器优先注册 `Ctrl+Alt+F`，冲突时依次尝试 `Ctrl+Alt+Shift+F` 和
`Ctrl+Alt+F8`。配置文件保存在 DLL 同级目录，使用 UTF-8 编码。

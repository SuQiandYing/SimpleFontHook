...
实现入口为 `font/font_patcher.h` 与对应的 `font_patcher.cpp`。复刻一个字体表行为时，
使用同一份 TTF、OTF、TTC 输入记录 SHA-256，并分别调用 `IsFontFile`、`IsFontCollection`、
目标补丁函数和校验和检查。每次输出保存字体头、表目录、修改表和最终哈希；失败输入保存
返回值与原始字节哈希。

| 输入 | 观测证据 | 预期输出 |
| --- | --- | --- |
| 独立 SFNT | `sfntVersion`、表目录范围检查通过 | 目标表按契约修改，目录和校验和一致 |
| TTC 多字体面 | `ttcf` 头、面数量、目标 `name` 记录 | 选择指定字体面并重建独立 SFNT |
| 缺失目标表 | 表目录没有目标标签 | 返回未修改，输入哈希保持一致 |
| 越界或损坏长度 | 范围检查失败 | 返回失败，不写入输出 |

复刻记录至少包含：输入路径和哈希、字体面选择条件、修改前后表目录、
`head.checkSumAdjustment`、返回状态和解析器版本。该记录对应
[功能证据与复刻流程](../../docs/reproduction.md) 的 `O/I/U` 标记。

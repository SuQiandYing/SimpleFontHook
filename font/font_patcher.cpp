...
static bool NormalizeGdiSfntData(std::vector<BYTE>& fontData) {
    if (fontData.size() < 12 || !IsSfntVersion(ReadU32BE(fontData.data()))) return false;

    WORD numTables = ReadU16BE(fontData.data() + 4);
    size_t directorySize = 12 + (size_t)numTables * 16;
    if (numTables == 0 || numTables > 256 || directorySize > fontData.size()) return false;

    size_t headOffset = 0;
    size_t headLength = 0;
    for (WORD i = 0; i < numTables; ++i) {
        const BYTE* entry = fontData.data() + 12 + (size_t)i * 16;
        if (ReadU32BE(entry) == 0x68656164) { // 'head'
            headOffset = ReadU32BE(entry + 8);
            headLength = ReadU32BE(entry + 12);
            break;
        }
    }
    if (headLength < 16) return false;

    size_t maxBias = std::min<size_t>(headOffset, 1024 * 1024);
    for (size_t bias = 0; bias <= maxBias; bias += 4) {
        size_t adjustedHead = headOffset - bias;
        if (adjustedHead > fontData.size() || headLength > fontData.size() - adjustedHead) continue;
        if (ReadU32BE(fontData.data() + adjustedHead) != 0x00010000 ||
            ReadU32BE(fontData.data() + adjustedHead + 12) != 0x5F0F3CF5) {
            continue;
        }

        bool valid = true;
        std::vector<BYTE> adjusted = fontData;
        for (WORD i = 0; i < numTables; ++i) {
            BYTE* entry = adjusted.data() + 12 + (size_t)i * 16;
            size_t offset = ReadU32BE(entry + 8);
            size_t length = ReadU32BE(entry + 12);
            if (offset < bias || offset - bias > adjusted.size() ||
                length > adjusted.size() - (offset - bias)) {
                valid = false;
                break;
            }
            WriteU32BE(entry + 8, (DWORD)(offset - bias));
        }
        if (!valid) continue;

        std::vector<BYTE> rebuilt;
        if (!ExtractSfntAtOffset(adjusted.data(), adjusted.size(), 0, rebuilt)) continue;
        fontData.swap(rebuilt);
        return true;
    }
    return false;
}
...
namespace FontPatcher {
    ...
    bool NormalizeGdiFontData(std::vector<BYTE>& fontData) {
        return NormalizeGdiSfntData(fontData);
    }

    bool FindFontCollectionFaceIndex(const std::vector<BYTE>& collectionData,
        const wchar_t* faceName, DWORD* outFaceIndex) {
        ...
        if (outFaceIndex) *outFaceIndex = static_cast<DWORD>(bestIndex);
        return true;
    }
    ...
    bool ExtractFontFromCollectionByName(const std::vector<BYTE>& collectionData,
        ...
        DWORD faceIndex = 0;
        if (!FindFontCollectionFaceIndex(collectionData, faceName, &faceIndex)) return false;

        std::vector<size_t> offsets;
        if (!GetFontOffsets(collectionData.data(), collectionData.size(), offsets) ||
            faceIndex >= offsets.size()) {
            return false;
        }
        return ExtractSfntAtOffset(collectionData.data(), collectionData.size(),
            offsets[faceIndex], outFontData);
    }
